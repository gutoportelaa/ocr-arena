...
class MarkerEngine(BaseOCREngine):
    ...
    def process_pdf(self, pdf_path: Path) -> List[OCRResult]:
        ...
        logger.info(f"Marker processing PDF: {pdf_path.name}")
        # Implementation would use marker library or CLI here
        return []

    def process_page(self, image_path: Path, page_number: int, pdf_name: str) -> OCRResult:
        return OCRResult(self.name, pdf_name, page_number, "", "", {}, False, "Page processing not implemented for Marker")
